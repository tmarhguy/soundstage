...
├── Sources/SoundStageCore/  testable policy (enable, clock, settings, login item)
...
- **Open at login:** Settings gear → `SMAppService.mainApp` Login Item (OS is the
  source of truth; not stored in mixer JSON). Auto-resume is unchanged: login
  launches the process; routing starts only if `wasRunning`. Translocated copies
  and unpackaged `swift run` cannot register. Presentation policy lives in
  `SoundStageCore` (`loginItemPresentation`) so XCTest can cover it without
  ServiceManagement. **User-facing guide:** [LOGIN.md](LOGIN.md).
- **Settings page:** same 360pt panel as the mixer (`mixer` | `settings`); gear
  is hidden during `--capture` / preview.
